(function($) {
	...
	$.fn.draw = function(o,obj,selectedpage){
		...
		var selobj;
		var pglbls = new Array();
		$('.cmpnt_grp').each(function(n) {
            pglbls[n] = $.trim($(this).html());
        });
		for(var i = 0; i < o.count; i++){
			var val = i+1;
			//RPS, modified to accommodate customized page labels with "pageLbl" var
			var pageLbl;
			if(pglbls.length > 0){
				//alert(pglbls.length);
				pageLbl = pglbls[i];
			...
			//alert('pageLbl has value of: ' + pageLbl);
			if(val == selectedpage){
				//RPS, modified to accommodate customized page labels with "pageLbl" var
				var _obj = $(document.createElement('li')).html('<span class="jPag-current">'+pageLbl+'</span>');
				selobj = _obj;
				_ul.append(_obj);
			}	
			else{
				//RPS, modified to accommodate customized page labels with "pageLbl" var
				var _obj = $(document.createElement('li')).html('<a>'+ pageLbl +'</a>');
				_ul.append(_obj);
			}				
		}		
		...
		_ulwrapdiv.find('li').click(function(e){
			...
			var currval;
			//RPS, modified to accommodate customized page labels with "pglbl" var and for loop.
			var pglbl = $(this).find('a').html();
			for(var i = 0; i < pglbls.length; i++){
				if(pglbls[i] == pglbl){
					currval = i+1;
				}
			}
			//alert('In _ulwrapdiv.find().click and currval is: '+currval);
			$(this).html('<span class="jPag-current">'+pglbl+'</span>');
			selobj = $(this);
			...
			o.onChange(currval);
			$('.inneraccordion .head.'+pglbl+'_hdr:not([class*=all_instances])').each(function() {
			...
			//end of RPS inneraccordion handling
		});
