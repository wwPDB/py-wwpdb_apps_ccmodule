...
# 2011-02-09    RPS    doRender_AtmMpList(), doRender_BatchSrchSummaryContent() and  doRender_BatchRslts() updated to improve handling of cases where no top hits found for given ligands.
# 2011-02-22    RPS    doRender_BatchSrchSummaryContent() updated as per new strategy of having Batch Search Summary View and Instance Searching View reside on single web page/browser window.
...
# 2011-03-31    RPS    doRender_EntityBrwsr() and doRender_EntityGrpOnRerunSrch() updated to accommodate proper handling of TMP_IDs when files are uploaded for processing on shared server
...
# 2011-05-17    RPS    doRender_InstanceProfileOnRerunSrch() removed, and doRender_AllInstncsProfile() and doRender_InstanceProfile() created in order to consolidate redundant blocks of code
...
class ChemCompAssignDepict(ChemCompDepict):
    ...
    def generateInstancesMainHtml(self, ccAssignDataStore, instanceIdList, linkInfoMap, reqObj):
        ...
        #
        hlprDict={}
        hlprDict['sessionid'] = r'%(sessionid)s'
    ...
    def _generateInstanceProfileHtml(self, ccAssignDataStore, hlprDict, rerun=False, reqObj=None):
        ...
        hlprDict['2dpath_labld_w_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&file={}.svg'.format(depId, wfInstId, instId, topHitCcId+'_Big')
        hlprDict['2dpath_labld_no_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&file={}.svg'.format(depId, wfInstId, instId, topHitCcId+'_Big')
        ...
        ##
        #
        hlprDict['2dpath']           = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&file={}.svg'.format(depId, wfInstId, instId, instId)
        hlprDict['2dpath_top_hit']   = '/service/cc/report/file?identifier={}&instance={}&source=ccd&ligid={}&file={}.svg'.format(depId, wfInstId, instId, topHitCcId)
        ...
        hlprDict['jmol_code_base'] = self.jmolCodeBase
        hlprDict['2dpath_labld_w_hy'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&file={}.svg'.format(depId, wfInstId, instId, instId+'_Big')
        hlprDict['2dpath_labld_no_hy'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&file={}.svg'.format(depId, wfInstId, instId, instId+'_Big')
        ############################################################################################################################################################
        #################################################################################################################################
    ...
    def doRender_InstanceProfile(self,p_ccAssgnDataStr,p_hlprDict,p_bRerun=False,p_reqObj=None):
        ...
        p_hlprDict['2dpath_labld_w_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, topHitCcId+'_Big')
        p_hlprDict['2dpath_labld_no_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, topHitCcId+'_Big')
        ...
        ##
        #
        p_hlprDict['2dpath']           = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, instId)
        p_hlprDict['2dpath_top_hit']   = '/service/cc/report/file?identifier={}&instance={}&source=ccd&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, topHitCcId)

        ##
        #
        ## added by ZF. It is needed by directly call from ChemCompWebApp._ccAssign_rerunInstncSrch && ChemCompWebApp._ccAssign_rerunInstncCompSrch
        p_hlprDict['jmol_code_base'] = self.jmolCodeBase
        p_hlprDict['2dpath_labld_w_hy'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, instId+'_Big')
        p_hlprDict['2dpath_labld_no_hy'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, instId, sessionId, instId+'_Big')
        ############################################################################################################################################################
        #################################################################################################################################
        ## below we are setting dictionary items that pertain to different display states depending
        ## on whether or not given ligand instance has been assigned a CC ID by annotator
    ...
    def doRender_InstanceNewCandidate(self,p_instId,p_ccId,p_ccAssgnDataStr,p_reqOb):
        ...
        for (ccId,name,formula) in p_ccAssgnDataStr.getNewCandidatesList(p_instId):
            ...
            if ccId == p_ccId:
                ...
                strReplDict['3dpath_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=ccd&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, p_ccId, sessionId, p_ccId)
                strReplDict['2dpath_labld_w_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, p_instId, sessionId, p_ccId+'_Big')
                strReplDict['2dpath_labld_no_hy_ref'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, p_instId, sessionId, p_ccId+'_Big')
                ...
                #################################################################################################
                htmlPathAbs = os.path.join(ccReportPath,'html',p_instId)
                jmolPathAbs = os.path.join(ccReportPath,'html',p_instId)
                ...
                #
                ## added by ZF.
                strReplDict['2dpath'] = '/service/cc/report/file?identifier={}&instance={}&source=report&ligid={}&sessionid={}&file={}.svg'.format(depId, wfInstId, p_instId, sessionId, p_ccId)
                if not os.path.exists(htmlPathAbs):
                    os.makedirs(htmlPathAbs)
                fp=open(htmlFilePathAbs,'w')
                fp.write("%s" % self.processTemplate(tmpltPth=os.path.join(htmlTmpltPth,self.__pathSnglInstcCmprTmplts),fn="cc_viz_cmp_li_tmplt_newcandidt.html",parameterDict=strReplDict) )
                fp.close()
                #
                '''
                #################################################################################################
    ...
    def doRender_EntityGrpOnRerunSrch(self,p_entityGrp,p_instncIdLst,p_ccAssgnDataStr,p_linkInfoMap,p_reqOb):
        ...
        for instId in p_instncIdLst:
            ...
            htmlFilePathAbs = os.path.join(htmlPathAbs,instId+'_viz_cmp_li.html')
            fp=open(htmlFilePathAbs,'w')
            fp.write("%s" % self.processTemplate(tmpltPth=os.path.join(htmlTmpltPth,self.__pathAllInstncsCmprTmplts),fn="cc_all_instncs_viz_cmp_li_tmplt.html",parameterDict=hlprDict) )
            fp.close()
            #
            oL.append( self.processTemplate(tmpltPth=os.path.join(htmlTmpltPth,self.__pathSnglInstcTmplts),fn="cc_instnc_disp_tmplt.html",parameterDict=hlprDict) )
        
        rerunHtmlPathAbs = os.path.join(ccReportPath,"html",p_entityGrp)
    ...
    def doRender_3dEnvironView(self,p_reqObj,p_bIsWorkflow):
        ...
        myD['filesource'] = fileSource
        myD['identifier'] = depId
        myD['3dpath'] = '/service/cc/report/file?identifier={}&instance={}&source=instance&sessionid={}&file={}'.format(depId, wfInstnc, sessionId, depId+'-jmol-mdl')
