...
import multiprocessing, os, sys, time, types, string, shutil, traceback, threading
...
class ChemCompAssign(object):
    ...
    def getDataForInstncSrch(self,p_srchIdsL,p_ccAssgnDataStr):
        ...
        if self.__verbose:
            ...
            self.__lfh.flush()
        #
        assignDirPath=os.path.join(self.__cI.get('SITE_DEPOSIT_STORAGE_PATH'),'deposit',depDataSetId.upper(),'cc_analysis')
        #if self.__verbose:
            ...
            for dpstrUpldD in dpstrUploadL:
                ...
                if self.__devTest is True:
                    ...
                    if( upldFileName == 'XYZ-sketch.sdf' ):
                        #upldFileName = 'D_1100200134_ccdef_P1.sdf.V2'
    ...
    def __registerFilePaths(self,p_contentType,p_upldFileName,p_upldFileType,p_ccADS,p_ligId,p_dpstrFileSource):
        ...
        try:
            ...
            fileHndlingCatalog = {'component-sketch':('getChemCompSketchFilePath','setDpstrSketchFile','setDpstrSketchFileWfPath'),'component-image':('getChemCompImageFilePath','setDpstrUploadFile','setDpstrUploadFileWfPath'),'component-definition':('getChemCompDefntnFilePath','setDpstrUploadFile','setDpstrUploadFileWfPath')}
            ccI=ChemCompDataImport(self.__reqObj,verbose=self.__verbose,log=self.__lfh)
            ...
            if( not self.__isWorkflow() ): # FOR STANDALONE TESTING PURPOSES
                ...
                # above call gets us path to supporting file based on filetype and partition number (which maps to specific ligand ID, with version number being highest version number available
            ...
            # RPS: found need to turn this off for testing 10/4/13 --- assert( wfFlName == p_upldFileName ), ("+%s.%s() -- ASSERTION ERROR: p_upldFileName is: [%s] but wfFlName is: [%s]\n" % (className, methodName, p_upldFileName, wfFlName) )
            #
    ...
    def __storeFile(self,p_contentType,p_ccADS,p_fileName,p_partitionId,p_fileType,p_ligId):
        ...
        methodName = sys._getframe().f_code.co_name
        #
        fileHndlingCatalog = {'component-sketch':('getChemCompSketchFilePath','setDpstrSketchFileWfPath'),'component-image':('getChemCompImageFilePath','setDpstrUploadFileWfPath'),'component-definition':('getChemCompDefntnFilePath','setDpstrUploadFileWfPath')}
        #
        ccE=ChemCompDataExport(self.__reqObj,verbose=self.__verbose,log=self.__lfh)
        #
