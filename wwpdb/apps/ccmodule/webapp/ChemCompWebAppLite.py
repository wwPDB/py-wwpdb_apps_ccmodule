...
class ChemCompWebAppLiteWorker(object):
    def __init__(self, reqObj=None, verbose=False,log=sys.stderr):
        ...
        self.__appPathD={'/service/environment/dump':                            '_dumpOp',
                         ...
                         '/service/cc_lite/file':                                '_getDepositorProvidedFile',
                         # report endpoints
    ...
    def _getDepositorProvidedFile(self):
        """Get files provided by the depositor which are
        stored in the deposit folder.
        """
        rC=ResponseContent(reqObj=self.__reqObj, verbose=self.__verbose,log=self.__lfh)
        ...
        if (self.__verbose):
            self.__lfh.write("+ChemCompWebAppLite._getDepositorProvidedFile() Requesting file '%s' from deposition '%s'\n" % (filename, depId))
        
        # allowing only alphanum, _ and . characters
        filename = re.sub('[^ a-zA-Z0-9_.-]+', '', filename)
        filePath = os.path.join(self.__pathInfo.getDepositPath(depId), filename)
