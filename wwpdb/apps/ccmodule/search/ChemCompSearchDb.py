...
class ChemCompSearchDb(object):
    ...
    def _runIdSearch(self,ccId,op):
        ...
        query = "select distinct chem_comp.Structure_ID,pdb_entry.method,pdb_entry.title,pdb_entry.date_of_RCSB_release,pdb_entry.status_code,pdb_entry.Structure_ID,chem_comp.id from chem_comp,pdb_entry  " + constraint        
        if self.__verbose:
    ...
    def _runIdHoldSearch(self,ccId,op):
        ...
        if (op == 'LIKE'):
            pc = "%"        
            tccId = pc + ccId + pc
            constraint = "where chem_comp.id LIKE \"%s\" and pdb_entry.status_code != \"REL\" and chem_comp.Structure_ID = pdb_entry.Structure_ID order by pdb_entry.initial_deposition_date DESC " % tccId
        else:
            constraint = "where chem_comp.id = \"%s\" and pdb_entry.status_code != \"REL\" and chem_comp.Structure_ID = pdb_entry.Structure_ID order by pdb_entry.initial_deposition_date DESC " % ccId
        
        query = "select distinct chem_comp.Structure_ID,pdb_entry.method,pdb_entry.title,pdb_entry.initial_deposition_date,pdb_entry.status_code,chem_comp.id from chem_comp,pdb_entry  " + constraint
    ...
    def doIdSearch(self):
        ...
        retDict={}
        if (targetId == None or len(targetId) < 1):
            return retDict
        #  
        if (self.__verbose):
